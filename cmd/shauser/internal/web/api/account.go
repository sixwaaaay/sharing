...
type AccountHandler struct {
	client *server.UserServiceServer
	signer Signer
}

func NewAccountHandler(client *server.UserServiceServer, signer Signer) *AccountHandler {
...
func (h *AccountHandler) Login(ctx echo.Context) error {
	...
	reply, err := h.client.Login(ctx.Request().Context(), &user.LoginRequest{
		Email:    req.Email,
...
func (h *AccountHandler) Register(ctx echo.Context) error {
	...
	reply, err := h.client.Register(ctx.Request().Context(), &user.RegisterRequest{
		Email:    req.Email,
		Password: req.Password,
		Name:     req.Username,
	})
...
func covert(u *user.User) *Account {
	return &Account{
		Id:            u.Id,
		Name:          u.Name,
		IsFollow:      u.IsFollow,
