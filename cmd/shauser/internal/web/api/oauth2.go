...
import (
	...
	pb "github.com/sixwaaaay/shauser/user"
)
...
func NewOauth2(conf *oauth2.Config, uc *server.UserServiceServer, signer Signer) *Oauth2 {
...
func (O *Oauth2) Callback(c echo.Context) error {
	...
	mail, err := O.uc.GetByMail(c.Request().Context(), &pb.GetByMailReq{
		Email: user.Email,
	})
	if err != nil {
		return err
	}
	// if user already exists, sign in directly and return redirect
	if mail.User != nil {
		signedToken, err := O.Signer(mail.User.Id, mail.User.Name)
		if err != nil {
			return err
		}
