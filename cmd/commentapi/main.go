...
func (h *Handler) Update(e *echo.Echo) {
	e.GET("/comments", h.CommentList, echo.WrapMiddleware(sign.Middleware(h.secret, false)))
	e.POST("/comments", h.Comment, echo.WrapMiddleware(sign.Middleware(h.secret, true)))
}
...
func (h *Handler) CommentList(ctx echo.Context) error {
	...
	resp, err := h.cli.CommentList(ctx.Request().Context(), &pb.CommentListReq{
		VideoId:   id,
		Token:     req.Token,
		Limit:     req.Limit,
		SubjectId: id,
	...
	reply := CommentListReply{
		Comments: resp.CommentList,
	}
...
func (h *Handler) Comment(ctx echo.Context) error {
	...
	resp, err := h.cli.CommentAction(ctx.Request().Context(), &pb.CommentActionReq{
		VideoId:     req.VideoID,
		Action:      req.Action,
