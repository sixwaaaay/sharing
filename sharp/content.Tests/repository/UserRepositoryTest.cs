...
public class UserRepositoryTest {
  ...
  private readonly ChannelBase _channel = GrpcChannel.ForAddress(
      Environment.GetEnvironmentVariable("USER_STRING")!);
  ...
  public async void Test3() {
    var client = new UserRepository(_channel) { Token = "token" };
    await Assert.ThrowsAsync<RpcException>(
        async () =>
            await client.FindAllByIds(new[] { 493764627922944111, 11 }));
  }
}