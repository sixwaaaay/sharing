...
public class DomainTest {
  ...
  public async Task FindById_ReturnsVideoDto_WhenVideoExists() {
    ...
    var service = new DomainService(mockVideoRepo.Object, mockUserRepo.Object,
                                    mockVoteRepo.Object);
  ...
  public async Task FindAllByIds_ReturnsVideoDtos_WhenVideosExist() {
    ...
    var mockUserRepo = new Mock<IUserRepository>();
    var mockVoteRepo = new Mock<IVoteRepository>();
    var videos = new List<Video> { new() { Id = 1, UserId = 1 },
                                   new() { Id = 2, UserId = 2 } };
    var users = new List<User> { new() { Id = 1 }, new() { Id = 2 } };
    mockVideoRepo.Setup(repo => repo.FindAllByIds(It.IsAny<long[]>()))
        .ReturnsAsync(videos);
    mockUserRepo.Setup(repo => repo.FindAllByIds(It.IsAny<IEnumerable<long>>()))
        .ReturnsAsync(users);
    mockVoteRepo.Setup(repo => repo.VotedOfVideos(It.IsAny<long[]>()))
  ...
  public async Task Save_ReturnsSavedVideoDto_WhenVideoIsSaved() {
    ...
    var video = new Video { Id = 1, UserId = 1 };
    var user = new User { Id = 1 };
    mockVideoRepo.Setup(repo => repo.Save(It.IsAny<Video>()))
        .ReturnsAsync(video);
    var service = new DomainService(mockVideoRepo.Object, mockUserRepo.Object,
                                    mockVoteRepo.Object);
