...
public static partial class UserExtension {
  ...
  public static partial IReadOnlyList<User>
  ProtoToUser(this IReadOnlyList<GrpcUser.User> users);
}
...
public class UserRepository : IUserRepository {
  ...
  public UserRepository(ChannelBase channel) {
    _client =
        new GrpcUser.UserService.UserServiceClient(channel.Intercept(Func));
  }
  ...
  public async Task<User> FindById(long id) {
    var request = new GrpcUser.GetUserRequest() { UserId = id };
  ...
  public async Task<IReadOnlyList<User>> FindAllByIds(IEnumerable<long> ids) {
    var request =
        new GrpcUser.GetUsersRequest() { UserIds = { ids.Distinct() } };
...
public enum VoteType { CancelVote, Vote }

public class VoteRepository
(HttpClient client) : IVoteRepository {
  ...
  public async Task UpdateVote(long videoId, VoteType type) {
    ...
    var resp =
        type switch { VoteType.Vote => await client.PostAsJsonAsync(
                          "/item/add", row, VoteJsonContext.Default.VoteRow),
                      VoteType.CancelVote => await client.PostAsJsonAsync(
                          "/item/delete", row, VoteJsonContext.Default.VoteRow),
  ...
  public async Task<IReadOnlyList<long>> VotedOfVideos(long[] videoIds) {
    ...
    var req = new ExistsReq() { Type = "vote", SubjectId = CurrentUser,
                                TargetIds = videoIds };
    var resp = await client.PostAsJsonAsync("/item/exists", req,
                                            VoteJsonContext.Default.ExistsReq);
    resp.EnsureSuccessStatusCode();
    var result = await resp.Content.ReadFromJsonAsync(
                     VoteJsonContext.Default.ExistsResp) ??
                 new ExistsResp();
    return result.Exists;
  }

  public async Task<IReadOnlyList<long>> VotedVideos(long userId, long page,
                                                     int size) {
    var req = new ScanReq() { Type = "vote", SubjectId = userId, Token = page,
                              Limit = size };
    var resp = await client.PostAsJsonAsync("/item/scan", req,
                                            VoteJsonContext.Default.ScanReq);
    resp.EnsureSuccessStatusCode();
    var result = await resp.Content.ReadFromJsonAsync(
                     VoteJsonContext.Default.ScanResp) ??
                 new ScanResp();
...
public static class Extension {
  ...
  AddVoteRepository(this IServiceCollection services) =>
      services.AddScoped<IVoteRepository, VoteRepository>();

  public static IServiceCollection
  AddUserRepository(this IServiceCollection services) =>
      services.AddScoped<IUserRepository, UserRepository>();

  public static IServiceCollection
  AddGrpcUser(this IServiceCollection services) =>
      services.AddSingleton<ChannelBase>(
          sp => GrpcChannel.ForAddress(
              sp.GetRequiredService<IConfiguration>().GetConnectionString(
                  "User") ??
              throw new InvalidOperationException(
                  @"User connection string is null.")));

  public static IServiceCollection
  AddVoteClient(this IServiceCollection
                    services) => services.AddSingleton<HttpClient>(sp => {
    var connectionString =
        sp.GetRequiredService<IConfiguration>().GetConnectionString("Vote") ??
  ...
  UseToken(this IApplicationBuilder app) => app.Use(async (context, next) => {
    var userRepository =
    ...
    var voteRepository =
        context.RequestServices.GetService<IVoteRepository>() ??
        throw new NullReferenceException();
