...
(MySqlDataSource dataSource) : IVideoRepository {
  ...
  public async Task<IReadOnlyList<Video>> FindAllByIds(long[] ids) {
    ...
    while (await result.ReadAsync()) {
      videos.Add(new Video {
        Id = result.GetInt64(0), UserId = result.GetInt64(1),
        Title = result.GetString(2), Des = result.GetString(3),
        CoverUrl = result.GetString(4), VideoUrl = result.GetString(5),
        Duration = result.GetInt32(6), Category = result.GetString(7),
        Tags = result.GetString(8), ViewCount = result.GetInt32(9),
        LikeCount = result.GetInt32(10), CreatedAt = result.GetDateTime(11),
        UpdatedAt = result.GetDateTime(12), Processed = result.GetInt16(13)
      });
    }
  ...
  public async Task<IReadOnlyList<Video>> FindByUserId(long userId, long page,
                                                       int size) {
    await using var connection = await dataSource.OpenConnectionAsync();
  ...
  public async Task<Video> Save(Video video) {
    ...
    var result = await connection.QuerySingleAsync<long>(
        ...
        new { video.UserId, video.Title, video.Des, video.CoverUrl,
              video.VideoUrl, video.Duration, video.Category, video.Tags,
              video.ViewCount, video.LikeCount, video.CreatedAt,
              video.UpdatedAt, video.Processed },
        transaction);
...
public static class VideoRepositoryExtensions {
  ...
  AddVideoRepository(this IServiceCollection services) =>
...
internal class InClause<T>(IEnumerable<T> values) {
  /// <summary>
  /// Gets the parameters for the IN clause, each with a unique name and a
  /// value.
  /// </summary>
  ...
  /// </summary>
  /// <param name="command">The SQL command to which the parameters will be
  /// added.</param> <returns>The same SQL command, for chaining
  /// calls.</returns>
